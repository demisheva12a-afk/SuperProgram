...
def ask(new_qustion):
	shuffle(answers)
	answers[0].setText(new_qustion.right_answer)
	answers[1].setText(new_qustion.w1)
	answers[2].setText(new_qustion.w2)
	answers[3].setText(new_qustion.w3)
	lb_question.setText(new_qustion.question)
	lb_answer.setText(new_qustion.right_answer)
	show_question()
...
'''Коли функція викликається, збільшуємо лічильник на 1.
Якщо номер поточного питання дорівнює довжині списку, скиньте лічильник. 
Отримайте питання зі списку, використовуючи номер. 
Задайте питання за допомогою функції ask(). 
'''
def next_question():
    ...
    new_question = 	question_list[window.cur_question]
    ask(new_question)
...
window.cur_question = -1
next_question()
btn_1.clicked.connect(click_OK)
